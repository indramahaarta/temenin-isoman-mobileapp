import 'package:tips_and_tricks/models/article.dart';

var listArticles = [
  Article(
    title: "COVID-19 Safety Tips for Families",
    source: "hopkinsmedicine",
    publishedDate: "2021-08-19",
    imageUrl:
        "https://www.hopkinsmedicine.org/-/media/images/health/1_-conditions/coronavirus/covid-family-safety-10.ashx?h=441&w=640&hash=8FB2C7006905C7E44FDF274F3F9AAB48",
    articleUrl:
        "https://www.hopkinsmedicine.org/health/conditions-and-diseases/coronavirus/covid19-safety-tips-for-families",
    briefDescription:
        "Although the coronavirus pandemic is not over, there is some good news: The COVID-19 vaccines are widely available, safe and very effective at preventing SARS-CoV-2 infection and the severe forms of COVID-19. Here are some recommended safety tips for your family from Johns Hopkins Children’s Center infectious diseases specialist Anna Sick-Samuels, M.D., M.P.H.",
  ),
  Article(
    title: "4 Cara Jaga Kesehatan Mental saat Pandemi COVID-19",
    source: "halodoc",
    publishedDate: "2021-01-15",
    imageUrl:
        "https://d1bpj0tv6vfxyp.cloudfront.net/articles/331057_15-1-2021_13-34-9.webp",
    articleUrl:
        "https://www.halodoc.com/artikel/4-cara-jaga-kesehatan-mental-saat-pandemi-covid-19",
    briefDescription:
        "Situasi pandemi COVID-19 yang terjadi kurang lebih 9 bulan lamanya telah mengubah beberapa sisi kehidupan kita. Mulai dari lebih giat menjaga kesehatan dan kebersihan diri, menjaga jarak, sampai berkegiatan di luar rumah dengan segala keterbatasan yang ada. Pandemi ini juga membuat kita lebih peduli dengan protokol kesehatan ketika keluar rumah di masa pandemi.",
  ),
  Article(
    title: "5 Cara Efektif agar Tidak Tertular Virus Corona",
    source: "Alodokter",
    publishedDate: "2020-06-04",
    imageUrl:
        "https://res.cloudinary.com/dk0z4ums3/image/upload/v1590522751/attached_image/ketahui-cara-untuk-mencegah-penularan-virus-corona-0-alodokter.jpg",
    articleUrl:
        "https://www.alodokter.com/ketahui-cara-untuk-mencegah-penularan-virus-corona",
    briefDescription:
        "Coronavirus telah menyerang ribuan orang di berbagai negara dan menelan ratusan korban jiwa. Agar tidak terkena penyakit infeksi yang sedang mewabah ini, ada beberapa cara yang bisa dilakukan untuk mencegah penularan virus Corona.",
  ),
  Article(
    title:
        "Cleaning and hygiene tips to help keep the COVID-19 virus out of your home",
    source: "Unicef",
    publishedDate: "2020-07-15",
    imageUrl:
        "https://www.unicef.org/indonesia/sites/unicef.org.indonesia/files/styles/hero_desktop/public/UNI306664.jpg?itok=Ry5EoLzO",
    articleUrl:
        "https://www.unicef.org/indonesia/coronavirus/cleaning-and-hygiene-tips-during-covid-19",
    briefDescription:
        "Can you catch the coronavirus disease (COVID-19) from food? How should I do laundry now? Mundane household tasks have turned into a source of uncertainty and anxiety as families grapple with getting the basics done all while keeping their loved ones safe and healthy. Widespread misinformation about the virus puts everyone at risk and adds to the stress of having to filter fact from fiction.",
  ),
  Article(
    title:
        "Menyongsong “New Normal” di Tengah Pandemi, Apa yang Bisa Kita Lakukan?",
    source: "sehatq",
    publishedDate: "2020-05-20",
    imageUrl:
        "https://cms.sehatq.com/public/img/article_img/menyongsong-new-normal-di-tengah-pandemi-apa-yang-bisa-kita-lakukan-1589973748.jpg",
    articleUrl:
        "https://www.sehatq.com/artikel/menyongsong-new-normal-di-tengah-pandemi-apa-yang-bisa-kita-lakukan",
    briefDescription:
        "Saat ini kita sudah dianjurkan untuk berdamai dengan corona dan menjalani the new normal. Sebenarnya apa yang harus kita lakukan saat memasuki fase ini? Berikut ini tipsnya dari dokter",
  ),
];
